...
class GetChargerComparisonUseCase @Inject constructor(
    ...
    /**
     * Aggregates charging sessions per charger profile and returns
     * a summary list sorted by most recently used.
     */
    operator fun invoke(): Flow<List<ChargerSummary>> {
        return combine(
            chargerDao.getChargerProfiles(),
                ...
                val avgTimeToFull = if (completedSessions.isNotEmpty()) {
                    completedSessions.mapNotNull { session ->
                        session.endTime?.let { end ->
                            ...
                            if (levelGain > 0) {
                                // Estimate time for full 0->100 charge based on rate
