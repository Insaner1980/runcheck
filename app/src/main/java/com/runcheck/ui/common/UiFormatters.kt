...
fun temperatureUnitRes(unit: TemperatureUnit): Int =
    when (unit) {
...
fun isUnknownValue(value: String?): Boolean =
    value.isNullOrBlank() || value?.compareTo("unknown", ignoreCase = true) == 0
...
fun connectionDisplayLabel(
    ...
    when (connectionType) {
        ConnectionType.WIFI -> {
            wifiSsid ?: stringResource(R.string.connection_wifi)
        }
...
@Composable
fun batteryHealthLabel(health: BatteryHealth): String =
    when (health) {
...
@Composable
fun plugTypeLabel(plugType: PlugType): String =
    when (plugType) {
...
@Composable
fun signalQualityLabel(quality: SignalQuality): String =
    when (quality) {
