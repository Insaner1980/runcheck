...
object LearnArticleCatalog {
    ...
    val sections: List<LearnTopicSection> = topicOrder.mapNotNull { topic ->
        articles.filter { it.topic == topic }
    ...
    private val legacyAliases: Map<String, String> = buildMap {
        articles.forEach { article ->
            article.legacyIds.forEach { legacyId ->
    ...
    fun resolveArticleId(id: String): String? = when {
