...
fun sanitizeFullscreenMetric(source: FullscreenChartSource, rawMetric: String?): String = when (source) {
    FullscreenChartSource.BATTERY_HISTORY -> sanitizeEnumName(
        rawValue = rawMetric,
        allowedNames = BatteryHistoryMetric.entries.map { it.name },
        defaultValue = BatteryHistoryMetric.LEVEL.name
    )
    FullscreenChartSource.BATTERY_SESSION -> sanitizeEnumName(
        rawValue = rawMetric,
        allowedNames = SessionGraphMetric.entries.map { it.name },
        defaultValue = SessionGraphMetric.CURRENT.name
    )
    FullscreenChartSource.NETWORK_HISTORY -> sanitizeEnumName(
        rawValue = rawMetric,
...
fun sanitizeFullscreenPeriod(source: FullscreenChartSource, rawPeriod: String?): String = when (source) {
    FullscreenChartSource.BATTERY_HISTORY -> sanitizeEnumName(
        rawValue = rawPeriod,
    ...
    FullscreenChartSource.NETWORK_HISTORY -> sanitizeEnumName(
        rawValue = rawPeriod,
        allowedNames = HistoryPeriod.entries
