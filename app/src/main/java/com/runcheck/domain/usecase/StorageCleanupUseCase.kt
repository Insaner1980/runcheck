...
class StorageCleanupUseCase @Inject constructor(
    ...
    suspend fun getCurrentStorageState(): StorageState =
        getStorageStateUseCase().first()

    suspend fun getTrashedUris(): List<String> =
        storageCleanupRepository.getTrashedUris()
    ...
    fun getCleanupItems(
        ...
        storageCleanupRepository.getCleanupItems(query, category)

    suspend fun getCleanupGroupUris(
        query: CleanupScanQuery,
        category: MediaCategory
    ): Set<String> =
        storageCleanupRepository.getCleanupGroupUris(query, category)
