    ...
    constructor(
        ...
        override fun getScreenUsageStats(): ScreenUsageStats? {
            synchronized(lock) {
                ...
                val onRatePerHour =
                    if (snapshot.screenOnDurationMs > 60_000) {
                        snapshot.screenOnDrainPct / (snapshot.screenOnDurationMs / 3_600_000f)
                    } else {
                        null
                    }
                val offRatePerHour =
                    if (snapshot.screenOffDurationMs > 60_000) {
        ...
        private fun loadStateOrCreate(now: Long): PersistedState {
            ...
            return PersistedState(
                ...
                lastIdleState =
                    prefs.getBoolean(
                        KEY_LAST_IDLE_STATE,
                        !powerManager.isInteractive && getCurrentIdleState(),
                    ),
                lastChargingStatus =
                    prefs
                        .getString(KEY_LAST_CHARGING_STATUS, null)
        ...
        private fun syncScreenState(
            ...
            if (state.screenOn != isScreenOn) {
                applyScreenStateChange(state, isScreenOn, now)
            } else {
                state
            }

        private fun syncIdleState(
            state: PersistedState,
            now: Long,
            isIdle: Boolean,
        ): PersistedState =
            if (!state.screenOn && state.lastIdleState != isIdle) {
                state.copy(
                    lastIdleCheckTime = now,
        ...
        private fun applyScreenStateChange(
            ...
            val drain =
                state.lastTransitionLevel?.let { startLevel ->
        ...
        private fun parseChargingStatus(value: String): ChargingStatus =
            ChargingStatus.entries.firstOrNull {
                it.name == value
        ...
        private data class PersistedState(
            ...
            fun snapshot(
                ...
                val currentDrain =
                    lastTransitionLevel?.let { startLevel ->
                        currentLevel?.let { (startLevel - it).coerceAtLeast(0).toFloat() }
                    } ?: 0f
