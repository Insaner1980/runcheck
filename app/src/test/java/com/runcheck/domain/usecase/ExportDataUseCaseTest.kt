...
class ExportDataUseCaseTest {
    ...
    @Test
    fun `CSV escaping - value with comma is quoted`() = runTest {
        coEvery { batteryRepository.getAllReadings() } returns listOf(
            BatteryReading(
    ...
    @Test
    fun `CSV escaping - value with quote is double-quoted`() = runTest {
        coEvery { batteryRepository.getAllReadings() } returns listOf(
            BatteryReading(
                timestamp = 1_700_000_000_000L,
                level = 80,
    ...
    @Test
    fun `empty battery data produces header row only`() = runTest {
        coEvery { batteryRepository.getAllReadings() } returns emptyList()
    ...
    @Test
    fun `empty network data produces header row only`() = runTest {
        coEvery { networkRepository.getAllReadings() } returns emptyList()
    ...
    @Test
    fun `empty thermal data produces header row only`() = runTest {
        coEvery { thermalRepository.getAllReadings() } returns emptyList()
    ...
    @Test
    fun `empty storage data produces header row only`() = runTest {
        coEvery { storageRepository.getAllReadings() } returns emptyList()
    ...
    @Test
    fun `battery export has correct column count`() = runTest {
        coEvery { batteryRepository.getAllReadings() } returns listOf(
            BatteryReading(
                timestamp = 1_700_000_000_000L,
                level = 80,
    ...
    @Test
    fun `battery export formats values correctly`() = runTest {
        coEvery { batteryRepository.getAllReadings() } returns listOf(
            BatteryReading(
                timestamp = 1_700_000_000_000L,
                level = 80,
    ...
    @Test
    fun `battery export handles null optional fields as empty`() = runTest {
        coEvery { batteryRepository.getAllReadings() } returns listOf(
            BatteryReading(
                timestamp = 1_700_000_000_000L,
                level = 80,
    ...
    @Test
    fun `network export has correct header`() = runTest {
        val csv = useCase.exportNetworkCsv()
    ...
    @Test
    fun `thermal export has correct header`() = runTest {
        val csv = useCase.exportThermalCsv()
        val header = csv.lines().first()
    ...
    @Test
    fun `storage export has correct header`() = runTest {
        val csv = useCase.exportStorageCsv()
        val header = csv.lines().first()
    ...
    @Test
    fun `exportAllCsv returns 4 entries`() = runTest {
    ...
    @Test
    fun `exportAllCsv values are non-empty strings`() = runTest {
        val result = useCase.exportAllCsv()
    ...
    fun `battery export throws for non-Pro user`() = runTest {
    ...
    @Test(expected = IllegalStateException::class)
    fun `network export throws for non-Pro user`() = runTest {
        every { proStatusProvider.isPro() } returns false
    ...
    @Test(expected = IllegalStateException::class)
    fun `thermal export throws for non-Pro user`() = runTest {
        every { proStatusProvider.isPro() } returns false
    ...
    @Test(expected = IllegalStateException::class)
    fun `storage export throws for non-Pro user`() = runTest {
        every { proStatusProvider.isPro() } returns false
    ...
    @Test
    fun `timestamps are formatted as ISO 8601`() = runTest {
        coEvery { batteryRepository.getAllReadings() } returns listOf(
            BatteryReading(
                timestamp = 1_700_000_000_000L,
                level = 80,
