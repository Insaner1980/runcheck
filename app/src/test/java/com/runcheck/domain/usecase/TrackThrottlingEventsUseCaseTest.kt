...
class TrackThrottlingEventsUseCaseTest {
    ...
    @Test
    fun `SEVERE thermal state opens throttling event`() =
        runTest {
    ...
    @Test
    fun `NONE after SEVERE closes event with duration`() =
        runTest {
            // Open the event
    ...
    @Test
    fun `duration is non-negative when closing event`() =
        runTest {
            useCase(thermalState(ThermalStatus.SEVERE))
            useCase(thermalState(ThermalStatus.NONE))
    ...
    @Test
    fun `peak status updates when thermal worsens from SEVERE to CRITICAL`() =
        runTest {
            useCase(thermalState(ThermalStatus.SEVERE))
    ...
    @Test
    fun `peak status updates through multiple escalations`() =
        runTest {
            useCase(thermalState(ThermalStatus.SEVERE))
    ...
    @Test
    fun `NONE when no active event is a no-op`() =
        runTest {
            useCase(thermalState(ThermalStatus.NONE))
    ...
    @Test
    fun `LIGHT when no active event is a no-op`() =
        runTest {
    ...
    @Test
    fun `MODERATE when no active event is a no-op`() =
        runTest {
    ...
    @Test
    fun `downgrade from CRITICAL to SEVERE does not close or update`() =
        runTest {
            useCase(thermalState(ThermalStatus.SEVERE))
            useCase(thermalState(ThermalStatus.CRITICAL))
    ...
    @Test
    fun `can open new event after closing previous one`() =
        runTest {
    ...
    @Test
    fun `EMERGENCY directly opens event`() =
        runTest {
            useCase(thermalState(ThermalStatus.EMERGENCY))
    ...
    @Test
    fun `foreground app is captured on insert and update`() =
        runTest {
    ...
    @Test
    fun `null foreground app is handled gracefully`() =
        runTest {
