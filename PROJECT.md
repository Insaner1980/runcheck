...
| Duplicate seen writes | Both ViewModels own an `UnseenInsightTracker`. It remembers the last unseen-ID set before scheduling a write; identical emissions do not automatically retry a failed write. An empty set resets the remembered set. |
...
- `Android14BatterySource` retains optional readable battery sysfs fallbacks for `cycle_count` and health derived from `charge_full / charge_full_design`. Health requires both charge values >=100,000, full <= design, and a result in 1..100. This is a battery-source implementation detail, not a public-API availability guarantee; the prohibition on thermal sysfs reads still applies. The UI's separate `designCapacityMah` field remains null.
...
| `security.yml` | Semgrep SAST on PRs/main plus OWASP Dependency-Check on weekly/manual runs | Semgrep `1.175.0` runs for push/PR and uploads SARIF through CodeQL Action `v4.37.6` except on fork and Dependabot PRs, whose read-only tokens cannot publish code-scanning results; setup-python is `v7.0.0`. OWASP stays out of push/PR analysis, runs scheduled/manually with setup-java `v5.7.0`, setup-gradle `v6.3.0`, cache action `v6.1.0`, a 45-minute job timeout, a 35-minute step timeout, and artifact upload `v7.0.1` when a report exists. |
| `sonar.yml` | SonarCloud scan through Gradle (`assembleDebug`, `:app:jacocoDebugUnitTestReport`, `sonar`) | Active on main pushes; checkout `v7.0.1`, setup-java `v5.7.0`, setup-android `v4.0.1` |
...
- This 2026-09-08 refresh changes only `PROJECT.md`. `UI-SPEC.md` retains the existing user's visual-system documentation; compare its claims with the implementation and the responsive/source-ownership details here. Do not interpret this date as a fresh validation of every companion document.
