...
## Yhteenveto
- Tavoite: samat PowerShell-lyhenteet toimivat `C:\Dev\runcheck`-repossa paikallisen `tools\*.ps1`-komentoperheen kautta ja käyttävät yhteistä Android-check-moduulia `tools\Invoke-RuncheckProjectCheck.ps1`-resolverin kautta.
...
- Jokainen wrapper on ohut välitin: se asettaa oikean `ProjectCheckCommand`-arvon, resolveeraa jaetun Android-checkin `ANDROID_CHECK_ROOT`-muuttujasta tai sisarhakemistosta, ja välittää alkuperäiset `@args`-argumentit eteenpäin.
